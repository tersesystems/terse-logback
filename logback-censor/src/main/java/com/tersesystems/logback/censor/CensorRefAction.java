...
public class CensorRefAction extends Action {
    ...
    public void begin(InterpretationContext ec, String tagName, Attributes attributes) {
        ...
        if (!(o instanceof CensorAttachable)) {
            String errMsg = "Could not find an CensorAttachable at the top of execution stack. Near [" + tagName + "] line " + getLineNumber(ec);
            inError = true;
        ...
        Map<String, Censor> censorBag = (Map<String, Censor>) ec.getObjectMap().get(CensorConstants.CENSOR_BAG);
        Censor censor = censorBag.get(censorName);

        if (censor == null) {
            String msg = "Could not find an censor named [" + censorName + "]. Did you define it below instead of above in the configuration file?";
            inError = true;
            addError(msg);
            return;
        }

        addInfo("Attaching censor named [" + censorName + "] to " + censorAttachable + "at " + getLineNumber(ec));
