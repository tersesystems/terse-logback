...
public class ExceptionMappingAction extends Action {
    ...
    public void begin(InterpretationContext ec, String tagName, Attributes attributes) {
        ...
        if (!(o instanceof ExceptionMappingRegistry)) {
            String errMsg = "Could not find a registry at the top of execution stack. Near ["
        ...
        ExceptionMapping newMapping = new BeanExceptionMapping(mappingName, mappingPropertyNames, handler);

        addInfo("Attaching mapping named [" + mappingName + "] to " + registry + "at " + getLineNumber(ec));
